...
def numerical_data_analysis(dataframe, feature: list):
    plt.hist(
        dataframe[dataframe["output"] == 1][feature],
        color="red",
        edgecolor="yellow",
        alpha=0.4,
    ...
    plt.hist(
        dataframe[dataframe["output"] == 0][feature],
        color="black",
        edgecolor="blue",
        alpha=0.4,
        label=f"{feature} - do not have disease",
    )
...
def remove_outliers(dataframe):
    ...
    outliers_free_data = dataframe[(zscore < 3).all(axis=1)]
    return pd.DataFrame(outliers_free_data)

def scale_dataset(data: pd.DataFrame, inverse: bool = False) -> np.ndarray:
    """
    Args
    --------
    data(pd.Dataframe): data set to scale down 
    inverse(bool): default = false 
    True if reverse the scale 
...
def getdiscretization(dataframe, feature: str, bins: list)->pd.DataFrame:
...
def model_building(model, x_train, y_train):
    ...
    shuffle_split = StratifiedShuffleSplit(
        train_size=0.8, test_size=0.2, n_splits=5, random_state=0
    )
